...
export class WebSocketAdapter implements MinimumCommunicationsTransport {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      console.log(`Comms connected! ${this.url}`)
        ...
        switch (message.$case) {
          ...
          case 'challengeMessage': {
            const authChainJson = JSON.stringify(
              Authenticator.signPayload(this.identity.authChain, message.challengeMessage.challengeToSign)
            )
            ws.send(
              craftMessage({
                message: {
