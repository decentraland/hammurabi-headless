  ...
  const handlePosition = (event: { address: string, data: any }) => {
    ...
    if (entity) {
      Transform.createOrReplace(entity, {
        position: new Vector3(d.positionX, d.positionY, d.positionZ),
        scale: Vector3.One(),
        rotation: new Quaternion(d.rotationX, d.rotationY, d.rotationZ, d.rotationW),
  ...
  const handleMovement = (event: { address: string, data: any }) => {
    ...
    if (entity) {
      Transform.createOrReplace(entity, {
        position: new Vector3(d.positionX, d.positionY, d.positionZ),
        scale: Vector3.One(),
        rotation: Quaternion.RotationAxis(Vector3.Up(), d.rotationY),
        parent: StaticEntities.GlobalCenterOfCoordinates
      })
    }
  }
