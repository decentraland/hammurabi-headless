...
/**
 * Regression tests for the security boundary: untrusted scene code runs inside
 * the QuickJS (WASM) sandbox and MUST NOT be able to reach the Node host realm
 * (which holds the worker's private key) nor wedge / exhaust the worker.
 *
 * These lock in the fix that replaced the escapable in-process `new Function`
 * runtime with QuickJS.
 */
describe('QuickJS scene sandbox isolation', () => {
  describe('when scene code probes for Node host globals', () => {
    it('should not expose the Node process object', async () => {
      const { result } = await withQuickJsVm(async (opts) => opts.eval(`typeof process`))
      expect(result).toBe('undefined')
    })

    it('should not expose a Node require on the global object', async () => {
      const { result } = await withQuickJsVm(async (opts) => opts.eval(`typeof globalThis.require`))
      expect(result).toBe('undefined')
    })
  })
  ...
  describe('when scene code attempts the classic Function("return this") realm escape', () => {
    it('should resolve to the VM global, which has no process (cannot reach the host)', async () => {
      const { result } = await withQuickJsVm(async (opts) =>
        opts.eval(`typeof Function('return this')().process`)
      )
      expect(result).toBe('undefined')
    })
  ...
  describe('when scene code runs a runaway synchronous loop', () => {
    it('should interrupt it at the execution deadline instead of hanging', async () => {
      await expect(
        withQuickJsVm(async (opts) => opts.eval(`while (true) {}`), { maxSyncExecutionMs: 200 })
      ).rejects.toThrow()
    })
  })
  ...
  describe('when scene code allocates past the memory ceiling', () => {
    it('should throw out-of-memory instead of exhausting the worker heap', async () => {
      await expect(
        withQuickJsVm(async (opts) => opts.eval(`const a = []; while (true) { a.push(new Array(100000).fill(0)) }`), {
