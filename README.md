...
| `HAMMURABI_MAX_TRACKED_PEER_SESSIONS` | `1024` | Max addresses whose live comms sessions are tracked (minimum 225); oldest non-live record evicted. |
| `HAMMURABI_MAX_SESSIONS_PER_PEER` | `8` | Max exact live session IDs retained per peer address; overflow liveness is tracked without retaining additional IDs. |
